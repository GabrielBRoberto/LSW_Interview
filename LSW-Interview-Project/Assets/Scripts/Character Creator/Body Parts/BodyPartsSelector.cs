...
public class BodyPartsSelector : MonoBehaviour
{
    ...
    public void NextBodyPart(int partIndex)
    {
        ...
        {
            if (bodyPartSelections[partIndex].bodyPartCurrentIndex < bodyPartSelections[partIndex].bodyPartOptions.Count - 1)
            {
    ...
    public void PreviousBody(int partIndex)
    {
        if (ValidateIndexValue(partIndex))
        {
            ...
            {
                bodyPartSelections[partIndex].bodyPartCurrentIndex = bodyPartSelections[partIndex].bodyPartOptions.Count - 1;
            }
    ...
    public void AddItem(SO_BodyPart bodyPart)
    {
        for (int i = 0; i < bodyPartSelections.Length; i++)
        {
            if (bodyPartSelections[i].bodyPartName == bodyPart.bodyPartCategory)
            {
                Debug.Log("Before: " + bodyPartSelections[i].bodyPartOptions.Count);
                bodyPartSelections[i].bodyPartOptions.Add(bodyPart);
                Debug.Log("After: " + bodyPartSelections[i].bodyPartOptions.Count);
            }
        }
    }
    public void RemoveItem(SO_BodyPart bodyPart)
    {
        for (int i = 0; i < bodyPartSelections.Length; i++)
        {
            if (bodyPartSelections[i].bodyPartName == bodyPart.bodyPartCategory)
            {
                Debug.Log("Before: " + bodyPartSelections[i].bodyPartOptions.Count);
                bodyPartSelections[i].bodyPartOptions.Remove(bodyPart);
                Debug.Log("After: " + bodyPartSelections[i].bodyPartOptions.Count);
            }
        }
    }
